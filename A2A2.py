    ...
    def changePDF(self, pdf_path):
        ...
        if sys.platform == 'win32' and 'sample' not in pdf_path:
            if "/" in pdf_path:
                ...
                with open("history.txt", "w", encoding='GB2312') as f:
                    config.write(f)
...
class MainWindow(QMainWindow,):
    ...
    def init_config(self):
        confile=open("config.ini","a")
        confile.close()
        config2 = configparser.ConfigParser()
        # -read读取ini文件
        config2.read("config.ini", encoding='UTF-8')
        list2 = config2.sections()# 获取到配置文件中所有分组名称
        if 'baidu' not in list2:# 如果分组type不存在则插入type分组
            config2.add_section('baidu')
            config2.set('baidu', 'appid', '')# 给type分组设置值
            config2.set('baidu', 'secretKey', '')# 给type分组设置值
        if 'Tencent' not in list2:# 如果分组type不存在则插入type分组
            config2.add_section('Tencent')
            config2.set('Tencent', 'appid', '')# 给type分组设置值
            config2.set('Tencent', 'secretKey', '')# 给type分组设置值
...
#pyinstaller --hidden-import=queue   A2A2.py -w